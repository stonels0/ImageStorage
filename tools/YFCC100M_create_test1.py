# -*- coding: utf-8 -*-
# @Author: Lishi
# @Date:   2018-06-08 19:39:32
# @Last Modified by:   Lishi
# @Last Modified time: 2018-06-21 21:37:43
import os,os.path
import sys
...
from submit2center import *                 # submit2center_test.py 文件，进行与数据库交互的功能;
import pdb
...
if __name__ == '__main__':
    ...
    demandlist1 = demandlist[0:12]
    demandlist2 = demandlist[12:24]
    demandlist3 = demandlist[24:36]
    demandlist4 = demandlist[36:48]
    demandlist5 = demandlist[48:60]
    demandlist6 = demandlist[60:70]
    #pdb.set_trace()
    task1 = threading.Thread(target = demo_YFCC100M, args = (demandlist1,))
    task1.start()
    task2 = threading.Thread(target = demo_YFCC100M, args = (demandlist2,))
    task2.start() 
    task3 = threading.Thread(target = demo_YFCC100M, args = (demandlist3,))
    task3.start()
    task4 = threading.Thread(target = demo_YFCC100M, args = (demandlist4,))
    task4.start() 
    task5 = threading.Thread(target = demo_YFCC100M, args = (demandlist5,))
    task5.start()
    task6 = threading.Thread(target = demo_YFCC100M, args = (demandlist6,))
    task6.start() 
"""
def testcode():
